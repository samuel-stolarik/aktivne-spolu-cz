...
async function vratPuvodni(
  ...
  const vysledek = await sHlasenim(
    hlaska,
    {
      probiha: "Vracím původní znění…",
      hotovo: "Hotovo. Na webu je zase původní znění.",
    },
    async () => {
      await zavolej("zrus-obsah", { klic: polozka.klic });
      delete prepisy[polozka.klic];
...
function vykresliPolozku(polozka: PolozkaObsahu): HTMLElement {
  const karta = document.createElement("div");
  karta.className = "karta-obsahu";
  ...
  hlaska.className = "hlaska-stavu mt-2";

  const prekresli = () => {
    const nova = vykresliPolozku(polozka);
    karta.replaceWith(nova);
  };
  ...
  if (polozka.typ === "obrazek") {
    ...
    vyber.addEventListener("change", () => {
      ...
      void (async () => {
        ...
        const vysledek = await sHlasenim(
          hlaska,
          {
            probiha: "Ukládám nový obrázek…",
            hotovo: "Uloženo. Na webu se obrázek objeví po obnovení stránky.",
          },
          async () => {
    ...
    if (jeUpraveno) {
      ...
      vratit.type = "button";
      vratit.className = "tlacitko-vedlejsi mt-3";
  ...
  if (jeUpraveno && prepisy[polozka.klic] !== polozka.vychozi) {
    ...
    puvodni.className = "puvodni-zneni";
  ...
  ulozit.type = "button";
  ulozit.className = "tlacitko-hlavni";
  ...
  if (jeUpraveno) {
    const vratit = document.createElement("button");
    vratit.type = "button";
    vratit.className = "tlacitko-vedlejsi";
